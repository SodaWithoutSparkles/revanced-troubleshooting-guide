...
class GitManager:
    ...
    def force_push_files(files, branch, commit_message):
        ...
        try:
            ...
            GitManager.run_git_command(
                f"git push origin HEAD:{branch} --force")
...
class ReVancedVersionUpdater:
    ...
    def parse_patches_data(self, patches_data):
        ...
        for package_name, versions in package_versions.items():
            version_list = sorted(
                list(versions), key=self.version_sort_key, reverse=True)
            result[package_name] = {
    ...
    def check_if_update_needed(self, current_data):
        ...
        try:
            with open(STATE_FILE, 'r') as f:
                ...
                last_yt_version = f.readline().strip()
    ...
    def find_markdown_files_with_placeholders(self):
        ...
        logging.info(
            f'Found {len(files_with_placeholders)} files with placeholders')
    ...
    def restore_backups(self):
        ...
        logging.info(
            f'Restoring {len(self.backup_file_paths)} files from backups...')
        ...
        for file_path in file_paths:
            try:
                ...
                if content != original_content:
                    ...
                    files_updated += 1
                    logging.debug(
                        f'Updated {file_replacements} placeholders in: {file_path}')

            except Exception as e:
                logging.warning(f'Failed to update {file_path}: {e}')

        logging.info(
            f'Updated {total_replacements} placeholders in {files_updated} files')
    ...
    def run(self):
        ...
        try:
            ...
            patches_version = version_data.get('version', 'unknown') if isinstance(
                version_data, dict) else 'unknown'
            ...
            _is_update_needed, update_reason, last_yt_version = self.check_if_update_needed(
                patches_data)
            if not (_is_update_needed or FORCED):
                ...
                return
            self.export_output(
                'yt_updated', 'true' if last_yt_version != youtube_version else 'false')
            # Determine final update reason
            final_update_reason = "forced_update" if FORCED else update_reason
            logging.debug(
                f"Reason: {_is_update_needed=}, {FORCED=}, final_reason={final_update_reason}")
            # Generate timestamp
            ...
            if not os.getenv('DRY_RUN'):
                ...
                if changes_made:
                    logging.info(
                        f"Successfully pushed updated files to {MAIN_BRANCH}")
                else:
                ...
                logging.info(
                    "DRY_RUN mode: Skipping file updates and git operations")
                changes_made = True
            ...
            logging.info(
                f'Update completed successfully. YouTube version: {youtube_version}')
            logging.info(
                f'Files updated: {files_updated}, Total replacements: {total_replacements}')
